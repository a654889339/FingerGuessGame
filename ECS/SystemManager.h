...
public:
    ...
    // 为什么这里是SystemObj, 而不直接用SystemBase, 
    // 因为这里如果要用SystemBase，就要确定SystemBase<Component>中的Component
    // 显然此工程作为框架，此时是无法确定<Component>的，
    // 因此使用SystemObj，一个超级基类代替SystemBase，而SystemObj是不需要<Component>的
...
private:
    ...
    // 因此我们对Update拆分为多阶段。举例：在SystemObj 存在Update Before, Update, Update After三个函数，具体实现由继承SystemObj的实例决定自己的函数实现。
